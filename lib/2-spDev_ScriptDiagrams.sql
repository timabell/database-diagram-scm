...
/**
<summary>
Restore diagram ''' + @thisdiagname + '''
</summary>
<remarks>
Generated by spDev_ScriptDiagrams/Tool_VarbinaryToVarcharHex
Will attempt to create [sysdiagrams] table if it doesn''t already exist
</remarks>
*/
PRINT ''=== spDev_ScriptDiagrams restore diagram [' + @thisdiagname + '] ===''
...
PRINT ''Create row for new diagram'''
			...
			SET @output=@output+'
BEGIN TRY
    PRINT ''Write diagram ' + @thisdiagname + ' into new row (and get [diagram_id])'''
...
BEGIN CATCH
    ...
    PRINT ''XxXxX END spDev_ScriptDiagrams - fix the error before running again XxXxX''
    RETURN
END CATCH
...
BEGIN TRY'
			...
			BEGIN
				...
				SELECT @line =  '
    UPDATE sysdiagrams SET [definition] .Write ( 0x' + UPPER(dbo.Tool_VarbinaryToVarcharHex (SUBSTRING (definition, @index, @chunk)))
					+ ', null, 0) WHERE diagram_id = @newid -- index:' + CAST(@index AS VARCHAR(100))
				FROM	sysdiagrams 
				WHERE	diagram_id = @diagram_id

				SET @output=@output+ISNULL(@line,'-- NULL Line2')
